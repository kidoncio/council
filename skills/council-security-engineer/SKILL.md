...
- **YAGNI & reuse (every advisor's job, through your lens).** Every speculative abstraction, unused flag, and parallel reimplementation is attack surface — more code to audit, more places a check can be forgotten, more drift between "the one that's wired up" and "the dead one." Flag complexity that no shipped slice exercises, and favor extending an existing, already-hardened asset over a new one.
