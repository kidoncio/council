...
- **YAGNI & reuse.** Distinguish scale the system genuinely faces from scale it never will. An abstraction, event log, or generalization that no shipped slice exercises is speculative complexity — a future migration cost, not a hedge. Lean, maintainable, reusable code is the default; flag the unexercised generality explicitly, even when your instinct is to design for 3 years out.
